...
public class DiscordHtmlTranscripts {
    ...
    public void returnFileUpload(TextChannel channel) throws IOException {
        returnFileUpload(channel, null);
    }

    public FileUpload returnFileUpload(TextChannel channel, String fileName) throws IOException {
        FileUpload htmlTranscript = FileUpload.fromData(generateFromMessages(channel.getIterableHistory().stream().collect(Collectors.toList())), fileName != null ? fileName : "transcript.html");
    ...
    public void createTranscript(TextChannel channel, String fileName) throws IOException {
        channel.sendFiles(FileUpload.fromData(generateFromMessages(channel.getIterableHistory().stream().collect(Collectors.toList())), fileName != null ? fileName : "transcript.html")).queue();

    }

    public File generateFromMessages(Collection<Message> messages) throws IOException {
        //File htmlTemplate = findFile("template.html");
        File htmlTemplate = null;
        try {
            URL url = new URL("https://raw.githubusercontent.com/Ryzeon/discord-html-transcripts/1.0/src/main/resources/template.html");
            URLConnection connection = url.openConnection();
            InputStream input = connection.getInputStream();
            htmlTemplate = new File("template.html");
            Files.copy(input, htmlTemplate.toPath(), StandardCopyOption.REPLACE_EXISTING);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        for (Message message : messages.stream()
            ...
            // message reference
            if (message.getMessageReference() != null) { // frag, ob es das ist
            ...
            timestamp
                    .text(message.getTimeCreated().format(DateTimeFormatter.ofPattern("HH:mm:ss").withZone(ZoneId.of("Europe/Berlin"))));
            ...
            messageContent.attr("title", "Message sent: "
                    + message.getTimeCreated().format(DateTimeFormatter.ofPattern("HH:mm:ss").withZone(ZoneId.of("Europe/Berlin"))));
...
//                System.out.println(message.getContentDisplay().contains("\r\n"));
                ...
                for (MessageEmbed embed : message.getEmbeds()) {
                    ...
                    if (embed.getFooter() != null) {
                        ...
                        embedFooterText.text(embed.getTimestamp() != null
                                ...
                                .format(DateTimeFormatter.ofPattern("HH:mm:ss").withZone(ZoneId.of("Europe/Berlin")))
        ...
        File htmlFertig = new File("transcript.html");
        FileUtils.copyInputStreamToFile(new ByteArrayInputStream(document.outerHtml().getBytes()), htmlFertig);
        return htmlFertig;
    }
